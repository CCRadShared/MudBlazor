...
namespace MudBlazor
{
    ...
    public partial class MudListExtended<T> : MudComponentBase, IDisposable
    {
        ...
        protected void HandleCentralValueCommander(string changedValueType, bool updateStyles = true)
        {
            ...
            {
                if (!MultiSelection && SelectedItem is not null)
                {
        ...
        protected internal void SetSelectedValue(T? value, bool force = false)
        {
            ...
            else
            {
                ...
                {
                    SelectedValues = SelectedValues?.Where(x => Comparer != null ? !Comparer.Equals(x, value) : x is not null ? !x.Equals(value) : true).ToHashSet(_comparer);
                }
                else
                {
        ...
        protected internal void SetSelectedValue(MudListItemExtended<T?>? item, bool force = false)
        {
            ...
            else
            {
                if (item.IsSelected)
                {
                    SelectedValues = SelectedValues?.Where(x => Comparer != null ? !Comparer.Equals(x, item.Value) : x is not null ? !x.Equals(item.Value) : true);
                }
                else
                {
                    if (SelectedValues == null)
...
#pragma warning disable BL0005
        ...
        public async Task ActiveFirstItem(string? startChar = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(startChar))
            {
                ...
                if (items[0].ParentListItem is not null && !items[0].ParentListItem.Expanded)
                {
